...
int CEditorView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_Font.CreateFontW(20, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, ANSI_CHARSET | DEFAULT_CHARSET | RUSSIAN_CHARSET,
...
{
	CPoint point = m_GraphicalWindow.m_Figure[m_GraphicalWindow.m_nSelectedFigure]->GetVertice(nVertice);
	CPoint pointCenter = m_GraphicalWindow.m_Figure[m_GraphicalWindow.m_nSelectedFigure]->GetCenter();
	if (bXOrY)
	{
		pointCenter.x -= point.x - nCoordinate;
	}
	else
	{
		pointCenter.y -= point.y - nCoordinate;
	}

	m_GraphicalWindow.MoveFigure(pointCenter);
